...
ax.set_ylabel("y")
ax.set_zlabel("z")
ax.set_title("lorenz system: True vs estimated motion using extended kalman filter")
true_line,=ax.plot([],[],[],'g-',label="true trajectory",lw=2)
pred_line,=ax.plot([],[],[],'r--', label="estimated trajectory", lw=2)
true_point,=ax.plot([],[],[],'go',label="true position")
pred_point,=ax.plot([],[],[],'ro',label="predicted position")
...
def init():
    ...
    return true_line, pred_line, true_point, pred_point

def update(frame):
    true_line.set_data(true_states[:frame,0],true_states[:frame,1])
    true_line.set_3d_properties(true_states[:frame,2])
    true_point.set_data([true_states[frame,0]],[true_states[frame,1]])
    true_point.set_3d_properties([true_states[frame,2]])

    pred_line.set_data(estimated_states[:frame,0],estimated_states[:frame,1])
    pred_line.set_3d_properties(estimated_states[:frame,2])
    pred_point.set_data([estimated_states[frame,0]],[estimated_states[frame,1]])
    pred_point.set_3d_properties([estimated_states[frame,2]])
    return true_line, pred_line, true_point, pred_point

animation=FuncAnimation(fig,update,frames=num_steps,init_func=init,blit=False,interval=10)
plt.show()
